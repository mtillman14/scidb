...
% Unmatched
data_path = "Y:\LabMembers\MTillman\SavedOutcomesAim2\Overground_EMG_Kinematics\MergedTablesAffectedUnaffected\unmatchedCycles.csv";
...
rootSavePath = 'Y:\LabMembers\MTillman\GitRepos\Stroke-R01-Aim-2\plots\UCM';
...
for varNum = 1:length(outcomeVarsNames)
    ...
    for subjectNum = 1:length(subjects)
        ...
        for interventionNum = 1:length(interventions)
            ...
            scatter(aggData(:,1), aggData(:,2), currColor, 'filled');
            xlabel('Affected');
            ylabel('Unaffected');
        ...
        legend([interventions; 'Perfect Symmetry']);
