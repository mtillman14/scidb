...
async function callFetch(method: string, params: Record<string, unknown>): Promise<unknown> {
  const routes: Record<string, { path: string | ((p: Record<string, unknown>) => string); method?: string; body?: boolean }> = {
    ...
    add_path_input_alternate:    { path: (p) => `/api/path-inputs/${encodeURIComponent(p.name as string)}/alternates`, method: 'POST', body: true },
    remove_path_input_alternate: { path: (p) => `/api/path-inputs/${encodeURIComponent(p.name as string)}/alternates/${encodeURIComponent(String(p.index))}`, method: 'DELETE' },
    deep_copy_path_input:   { path: (p) => `/api/path-inputs/${encodeURIComponent(p.node_id as string)}/deep-copy`, method: 'POST' },
    get_sweeps:              { path: '/api/sweeps' },
    create_sweep:            { path: '/api/sweeps', method: 'POST', body: true },
    update_sweep:            { path: (p) => `/api/sweeps/${encodeURIComponent(p.name as string)}`, method: 'PUT', body: true },
    delete_sweep:            { path: (p) => `/api/sweeps/${encodeURIComponent(p.name as string)}`, method: 'DELETE' },
