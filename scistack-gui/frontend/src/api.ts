...
async function callFetch(method: string, params: Record<string, unknown>): Promise<unknown> {
  const routes: Record<string, { path: string | ((p: Record<string, unknown>) => string); method?: string; body?: boolean }> = {
    ...
    hide_constant_value:    { path: (p) => `/api/constants/${encodeURIComponent(p.name as string)}/hidden_values/${encodeURIComponent(p.value as string)}`, method: 'POST' },
    unhide_constant_value:  { path: (p) => `/api/constants/${encodeURIComponent(p.name as string)}/hidden_values/${encodeURIComponent(p.value as string)}`, method: 'DELETE' },
    list_hidden_constant_values: { path: '/api/constants/hidden_values' },
