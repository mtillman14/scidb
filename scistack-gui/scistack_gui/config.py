...
class SciStackConfig:
    ...
    """Extra MATLAB path entries (directories to addpath)."""
...
def load_config(project_path: Path | None, db_path: Path) -> SciStackConfig:
    ...
    if section is None:
        raise ValueError(
            f"{toml_path} does not contain a scistack configuration section."
        )
    ...
    matlab_addpath: list[Path] = []
    for entry in matlab_section.get("addpath", []):
        p = (project_root / entry).resolve()
        if not p.is_dir():
            logger.warning("MATLAB addpath directory not found: %s", p)
        matlab_addpath.append(p)
...
def _resolve_glob_paths(
    ...
    for entry in raw_entries:
        # Support glob patterns (e.g. "matlab/types/*.m")
        if any(c in entry for c in ("*", "?", "[")):
            matched = sorted(
                Path(p) for p in _glob.glob(str(project_root / entry), recursive=True)
            )
            if not matched:
                logger.warning("%s glob matched no files: %s", label, entry)
